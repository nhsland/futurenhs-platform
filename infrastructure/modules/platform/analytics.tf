...
resource "azurerm_storage_account" "analytics" {
  name = "analytics${random_string.random_string_10_chars.result}"
  count = var.enable_analytics ? 1 : 0
  resource_group_name = azurerm_resource_group.platform.name
  location = var.location
...
resource "azurerm_mssql_server" "synapse" {
  ...
  administrator_login_password = azurerm_key_vault_secret.synapse_password[0].value
...
resource "azurerm_sql_database" "synapse_data_warehouse" {
  ...
  resource_group_name = azurerm_resource_group.platform.name
  location = var.location
  server_name = azurerm_mssql_server.synapse[0].name
...
//resource "azurerm_data_factory_linked_service_azure_blob_storage" "analyticsfactoryinput" {
//  name                = "analyticsfactoryinput"
//  resource_group_name = azurerm_resource_group.platform.name
//  data_factory_name   = azurerm_data_factory.analytics.name
//  connection_string   = azurerm_storage_account.analytics.primary_connection_string
////  connection_string   = data.azurerm_storage_account.example.primary_connection_string
//
//}
//
//resource "azurerm_data_factory_dataset_azure_blob" "analytics" {
//  name                = "analytics"
//  resource_group_name = azurerm_resource_group.platform.name
//  data_factory_name   = azurerm_data_factory.analytics.name
//  linked_service_name = azurerm_data_factory_linked_service_azure_blob_storage.analyticsfactoryinput.name
//
//  path_pattern = "analytics"
//  filename = "bar.png"
//}