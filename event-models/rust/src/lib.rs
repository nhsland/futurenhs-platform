...
impl<'de> Deserialize<'de> for CustomEvent {
        ...
        D: de::Deserializer<'de>,
    {
        ...
        impl<'de> de::Visitor<'de> for CustomEventVisitor {
            type Value = CustomEvent;

            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str("struct CustomEvent")
            }

            fn visit_map<V>(self, mut map: V) -> Result<CustomEvent, V::Error>
            where
                V: de::MapAccess<'de>,
            {
                let mut id = None;
                ...
                let data = match (event_type, data_version) {
                    ("loggedin", "1") => CustomEventData::LoggedIn(
                        serde_json::from_value(data).map_err(de::Error::custom)?,
                    ),
                    ("loggedin", "2") => CustomEventData::LoggedInV2(
                        serde_json::from_value(data).map_err(de::Error::custom)?,
                    ),
                    ("loginfailed", "1") => CustomEventData::LoginFailed(
                        serde_json::from_value(data).map_err(de::Error::custom)?,
                    ),
                    (a, b) => {
...
pub fn custom_parse() -> Result<CustomEvent, Box<dyn std::error::Error>> {
    println!(
        "{}",
    ...
    let event: CustomEvent = serde_json::from_str(
