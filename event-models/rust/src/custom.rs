        ...
        impl<'de> de::Visitor<'de> for EventVisitor {
                ...
                V: de::MapAccess<'de>,
            {
                ...
                let data = match (event_type, data_version) {
                    // TODO: This would have to be generated. Possibly with some kind of macro
                    ("loggedin", "1") => EventData::LoggedIn(
                        serde_json::from_value(data).map_err(de::Error::custom)?,
                    ),
                    ("loggedin", "2") => EventData::LoggedInV2(
                        serde_json::from_value(data).map_err(de::Error::custom)?,
                    ),
                    ("loginfailed", "1") => EventData::LoginFailed(
                        serde_json::from_value(data).map_err(de::Error::custom)?,
                    ),
                    (a, b) => {
